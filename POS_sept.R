...
#load dataset from ATM_sept.xlsx
ATM_sept <- read_xlsx("ATM_sept.xlsx")
index <- complete.cases(ATM_sept)
ATM_sept <- as.data.frame(ATM_sept[index, ])

#subset data to features we wish to keep/use and rename the same.
POS_sept <- ATM_sept[, c(2,5,6,9,11,14,16)]
...
colnames(POS_sept) <- col_names
POS_sept <- POS_sept %>% mutate(month = "sept") # adding new column "month".

#convert character to #1 integer and #2 double.
int_col <- c(2,3,4,6)
double_col <- c(5,7)
POS_sept[, int_col] <- lapply(POS_sept[, int_col], as.integer)
POS_sept[, double_col] <- lapply(POS_sept[, double_col], as.double)
